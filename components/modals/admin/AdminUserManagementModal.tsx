...
export const AdminUserManagementModal: React.FC<AdminUserManagementModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && (isSuperUser || (isOrganizationAdmin && organizationId))) {
      loadPlayers();
      setResettingUserId(null);
      setMessage(null);
      setNewPassword('');
    }
  }, [isOpen, isSuperUser, isOrganizationAdmin, organizationId]);
  ...
  const canManage = isSuperUser || (isOrganizationAdmin && !!organizationId);
          ...
          <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                {isSuperUser ? 'Recuperação de Acesso e Exclusão Global' : 'Gerenciamento de Membros da Organização'}
              </p>
            </div>
                ...
                <div key={p.id} className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-slate-400 text-xs">+55 {p.phone} • {isSuperUser ? (p.is_admin ? 'Administrador Global' : 'Jogador') : (orgIsAdmin ? 'Admin da Org' : 'Membro')}</p>
                  ...
                  {isSuperUser && resettingUserId === p.id && (
